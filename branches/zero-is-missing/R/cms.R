...
### main func
    ...
    rwi.firstyear <- yrs[1]
    c.vec <- rep(NA_real_, n.col)
    names(c.vec) <- col.names
    if (c.hat.t) {
        c.curve.mat <- matrix(NA_real_, ncol=n.col, nrow=n.row + max(po[[2]]))
    ...
    for (i in seq_len(n.col)) {
        ...
        if (length(no.na) > 0) {
            ...
            c.curve <- tmp[[1]]
            if (c.hat.t) {
                c.curve.mat[no.na + the.po, i] <- c.curve
            }
            rwi[[i]][no.na + (series.yrs[1] - rwi.firstyear)] <-
                series.no.na / c.curve
        }
    }
    ## export options
    if (c.hat.t) {
