qa.xdate <- function(rwl, seg.length, n, bin.floor){
    ...
    if (as.logical(seg.length %% 2)) {
        stop("'seg.length' must be even")
    }
    if (!is.int(seg.length)) {
        stop("'seg.length' and 'seg.lag' must be integers")
    }
    if (seg.length <= 0) {
        stop("'seg.length' must be positive")
    }
    ## n
    if (!is.null(n)) {
        if (!is.int(n)) {
